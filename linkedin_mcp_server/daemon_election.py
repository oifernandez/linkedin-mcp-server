...
def obtain_owner(
    ...
    *,
    ...
    Active election and quiescent settlement have separate budgets, but both
    deadlines are fixed from this call's entry. Settlement never starts another
    owner or asks one to stand down; it only observes the canonical descriptor
    and authenticates a compatible endpoint that answers before its deadline.
    """
    entered = time.monotonic()
    active_deadline = entered + max(deadline_seconds, 0.0)
    settlement_deadline = active_deadline + max(settlement_seconds, 0.0)
    started = False
    starts = 0
    next_start = 0.0
    ...
    while True:
        ...
        if attempt is _Attempt.STARTED:
            started = True
            # The only intentional inspector change: a proved commit superseded any
            # read that was already in flight against the predecessor generation.
            inspector = _DescriptorInspector(auth_root, profile, config)
