...
class TestElectionSettlement:
    def test_late_publish_after_active_deadline_converges(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ):
        profile = _profile(tmp_path)
        config = _config(profile)
        ...
        def contend(*args: object, **kwargs: object) -> _Attempt:
            nonlocal started
            if not started:
                started = True
...
class TestOwnerBootstrapOrdering:
    ...
    def test_winner_takes_lock_before_log_and_logging(
        ...
        assert daemon_owner.main(["--job-name", "test-owner-job"]) == 0
        assert events.index("lock") < events.index("log") < events.index("logging")
